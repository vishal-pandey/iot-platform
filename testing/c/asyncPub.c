...
#define PAYLOAD     "MEssage from async c program!"
#define QOS         1
...
int main(int argc, char* argv[])
{
    MQTTAsync client;
    MQTTAsync_connectOptions conn_opts = MQTTAsync_connectOptions_initializer;
    MQTTAsync_SSLOptions ssl_opts = MQTTAsync_SSLOptions_initializer; // ssl initializer 

    MQTTAsync_message pubmsg = MQTTAsync_message_initializer;
    MQTTAsync_token token;
    int rc;
    int a=MQTTAsync_create(&client, ADDRESS, CLIENTID, MQTTCLIENT_PERSISTENCE_NONE, NULL);
    // void * context;
    printf("%d\n",a );
    int xx = MQTTAsync_setCallbacks(client, NULL, connlost, NULL, NULL);
